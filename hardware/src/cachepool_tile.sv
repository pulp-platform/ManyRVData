...
module cachepool_tile
  ...
  // Enlarged address width for Spatz due to cache; single source in
  // cachepool_hpd_pkg (imported), also used as the private-L1 HPDcache paWidth.
  ...
  //                                             //
  // Moved to cachepool_hpd_pkg (imported above) //
  // and shared with cachepool_l1_ctrl so the    //
  // HPDcache config/types have one definition.   //
  ///////////////////////////////////////////////
  ...
  for (genvar c = 0; c < NumLP1CacheCtrl; c++) begin : gen_lp1_cache
    // HPDcache config & types (HPDcacheCfg, hpdcache_*_t, tcdm_meta_t,
    // downstream_info_t, word_offset_t, ...) are imported from cachepool_hpd_pkg
    // by both this tile and cachepool_l1_ctrl, so they no longer need threading.
    cachepool_l1_ctrl #(
  ...
  for (genvar j = 0; j < NumL2Plane; j++) begin : gen_cache_xbar
    ...
    if (NumRemoteGroupPortCore > 0) begin : gen_remote_group_slice
      ...
      // Collapsed, cacheline-wide data width.
      tcdm_cache_interco #(
        .NumTiles              (NumTiles                  ),
        .NumCores              (NrCores                   ),
        .NumCache              (NumL1CtrlTile             ),
