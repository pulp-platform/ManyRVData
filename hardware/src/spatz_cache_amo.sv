...
#(
  ...
  always_comb begin
    ...
    // An SC must always present its outcome on the *write* channel,
  ...
  always_comb begin : output_req_comb
    ...
    // This unit now sits below HPDcache's uncached/AMO handler, which expects BOTH an
    ...
    // instead of being filtered; forwarding is unconditional.
    // if (amo_rsp_valid && mem_rsp_i.p.user.is_amo) begin
  ...
  // LR arrives on the read channel and carries no byte-enables, so exclude it
