...
def emit_gemv_layer(name="gemv", **kwargs):
    ...
    if dtype != "char":
        layer_str += (
            f'static {dtype} {name}_A_dram [{m} * {n}] __attribute__((section(".pdcp_src"), used)) = '
            + array_to_cstr(mat_A)
            + ";\n\n\n"
        )
        layer_str += (
            f'static {dtype} {name}_result [{m}] __attribute__((section(".pdcp_src"), used)) = '
            + array_to_cstr(result)
            + ";\n\n\n"
        )
        layer_str += (
            f'static {dtype} {name}_B_dram [{n}] __attribute__((section(".pdcp_src"), used)) = '
            + array_to_cstr(vec_B)
            + ";\n"
        )
