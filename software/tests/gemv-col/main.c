...
int main() {
  ...
  // TODO: This unrolling algorithm does not work well, disable for now
  // if (m_core >= 64) {
  //   // Means we have enough element per core to unroll in M direction
  //   unroll_m = 1;
  //   // devide m_core by 2 to give space for unrolling
  //   // this would not influence the pointer calculation
  //   m_core   = (m_core >> 1);
  // }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (sizeof(T) == 8)
      if (unroll_m)
        gemv_v64b_m4_unroll_M(a_core, b, result_core, gemv_l.M, m_core, gemv_l.N);
      else
