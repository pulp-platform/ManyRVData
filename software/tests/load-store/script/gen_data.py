...
def emit_GEMM_layer(name='gemm', **kwargs):
    ...
    layer_str += f'static {dtype} {name}_A_dram [{m}*{k}]  __attribute__((section(".data")))  = ' + array_to_cstr(kwargs['A'], kwargs['prec']) + ';\n\n\n'
    layer_str += f'static {dtype} {name}_B_dram [{k}*{n}]  __attribute__((section(".data")))  = ' + array_to_cstr(kwargs['B'], kwargs['prec']) + ';\n\n\n'
    layer_str += f'static {dtype} {name}_C_dram [{m}*{n}]  __attribute__((section(".data")))  = ' + array_to_cstr(kwargs['C'], kwargs['prec']) + ';\n\n\n'
    layer_str += f'static {dtype} {name}_checksum[{m}]     __attribute__((section(".data")))  = ' + array_to_cstr(torch.sum(result, dim=-1), kwargs['prec']) + ';\n\n\n'
    return layer_str
